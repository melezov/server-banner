...
object Slant:
  ...
  private[serverbanner] class Glyph private(offset: Int, val parent: Option[Glyph]):
    ...
    override def toString: String =
      ...
      for y <- 0 until Glyph.Height do
        for x <- 0 to lineEndings(y) do
          sb += charAt(x, y)
        sb += '\n'
      sb.toString
  ...
  def apply(text: String): String =
    ...
    def writeChar(ch: Char, last: Option[Char], prev: Option[Char], pos: Int): Int =
      ...
      val lastLineEndings = if last.isDefined then
        val offsets = glyph.parent.get.lineEndings
        if _push then offsets.map(_ - 1) else offsets
      else
        NoPrev
